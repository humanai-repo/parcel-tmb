...
async function submitJobSpecs(jobSpecs: JobSpec[], parcel: Parcel): Promise<DocumentId[]> {
  ...
  do {
    ...
    for (const job of jobs) {
      ...
      const jobId = job.id;
      jobRunningOrPending =
        jobRunningOrPending || job.status.phase === JobPhase.PENDING || job.status.phase === JobPhase.RUNNING;
  ...
  const outputAddresses: DocumentId[] = [];
  for (const job of jobs) {
    if (job.status !== undefined) {
      for (const outputDoc of job.status.outputDocuments) {
